...
# Paths - Use EXPANDED masks (run EXPAND_MASKS_TO_FILES.py first if using 10fps SAM2!)
MASKS_FOLDER = r"D:\watermarkz\temp_sam2_masks_expanded"
MASKS_10FPS = r"D:\watermarkz\temp_sam2_masks"  # Original 10fps masks
...
def main():
    ...
    # Check masks - prefer expanded, fall back to 10fps
    if not os.path.exists(MASKS_FOLDER):
        if os.path.exists(MASKS_10FPS):
            print(f"[!] Expanded masks not found at {MASKS_FOLDER}")
            print(f"[!] Run EXPAND_MASKS_TO_FILES.py first to expand 10fps masks!")
            print(f"[!] Or using 10fps masks directly from {MASKS_10FPS}")
            # Use 10fps masks directly
            MASKS_FOLDER_ACTUAL = MASKS_10FPS
        else:
            print(f"ERROR: No masks in {MASKS_FOLDER}")
            print("Run SAM2 first to generate masks, then EXPAND_MASKS_TO_FILES.py!")
            return
    else:
        MASKS_FOLDER_ACTUAL = MASKS_FOLDER

    mask_files = sorted([f for f in os.listdir(MASKS_FOLDER_ACTUAL) if f.endswith('.png')])
    ...
    for i, mask_file in enumerate(mask_files):
        src = Path(MASKS_FOLDER_ACTUAL) / mask_file
