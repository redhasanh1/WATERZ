...
def process_video_task(self, video_path):
    ...
    try:
        ...
        if use_cropping and segments:
            ...
            for seg_idx, (start_frame, end_frame, seg_bbox) in enumerate(segments):
                seg_duration = end_frame - start_frame + 1
                print(f"\n🎬 Processing segment {seg_idx+1}/{len(segments)}: frames {start_frame}-{end_frame} ({seg_duration} frames)")

                # Calculate crop region with padding
                crop_region = calculate_crop_region(seg_bbox, width, height, padding_ratio=0.2, min_size=128)
                ...
                # Create directories for this segment
                seg_frames_dir = os.path.join(TEMP_DIR, f"{base_name}_{unique_suffix}_seg{seg_idx}_frames")
                seg_cropped_dir = os.path.join(TEMP_DIR, f"{base_name}_{unique_suffix}_seg{seg_idx}_cropped")
                seg_mask_dir = os.path.join(TEMP_DIR, f"{base_name}_{unique_suffix}_seg{seg_idx}_masks")
                seg_output_dir = os.path.join(TEMP_DIR, f"{base_name}_{unique_suffix}_seg{seg_idx}_output")
                seg_cleaned_dir = os.path.join(TEMP_DIR, f"{base_name}_{unique_suffix}_seg{seg_idx}_cleaned")
                ...
                os.makedirs(seg_cleaned_dir, exist_ok=True)

                # Copy segment frames from original_frames_dir
                # Note: -start_number 0 makes FFmpeg use 0-based numbering (frame 0 -> 0000.png)
                frames_copied = 0
                for frame_idx in range(start_frame, end_frame + 1):
                    src = os.path.join(original_frames_dir, f"{frame_idx:04d}.png")
                    dst = os.path.join(seg_frames_dir, f"{frames_copied:04d}.png")

                    # Safety check: skip if frame doesn't exist (handles edge cases)
                ...
                # Crop frames to watermark region (only process frames that were actually copied)
                ...
                for frame_idx in range(frames_copied):
                    ...
                    frame = cv2.imread(os.path.join(seg_frames_dir, frame_file))
                    if frame is None:
                ...
                # Crop masks to watermark region (only for frames that were actually copied)
                masks_copied = 0
                for frame_idx in range(start_frame, end_frame + 1):
                ...
                # Run faster-propainter pipeline on cropped segment  
                print(f"   🎨 Running faster-propainter pipeline on cropped segment...")
                progress = 55 + int((seg_idx / len(segments)) * 25)
                self.update_state(state='PROCESSING', meta={'progress': progress, 'status': f'faster-propainter segment {seg_idx+1}/{len(segments)}'})
                ...
                try:
                    ...
                    # Dynamic optimization for cropped segments - use higher quality since they're small
                    dynamic_subvideo, _ = get_dynamic_subvideo_length(crop_w, crop_h)
                    ...
                    # Direct pipeline call for cropped segment - TRUE FASTER-PROPAINTER SETTINGS
                    faster_propainter_pipeline(
                        video=seg_cropped_dir,              # Cropped frames directory
                        mask=seg_mask_dir,                  # Cropped masks directory
                        output=seg_output_dir,              # Segment output directory
                        resize_ratio=1.0,                   # Keep crop resolution
                        mask_dilation=4,                    # Standard mask dilation
                        ref_stride=10,                      # faster-propainter: reduce references
                        neighbor_length=20,                 # faster-propainter: recommended neighbors for quality
                        subvideo_length=80,                 # faster-propainter: standard subvideo length
                        raft_iter=20,                       # faster-propainter: standard optical flow
                        mode="video_inpainting",            # Standard inpainting
                        save_frames=True,                   # Save frames for merging
                    ...
                    print(f"❌ faster-propainter failed on segment {seg_idx}: {e}")
                    import traceback
                    traceback.print_exc()
                    raise RuntimeError(f"faster-propainter failed on segment {seg_idx}: {e}")
                finally:
                    # Clear GPU memory after each segment
                    clear_gpu_memory()

                # Find ProPainter output frames
                ...
                # Merge cleaned crops back to full frames
                ...
                # Convert segment frames to video
                seg_video_path = os.path.join(TEMP_DIR, f"{base_name}_{unique_suffix}_seg{seg_idx}.mp4")
                print(f"   🎞️  Encoding segment to video...")
                encode_cmd = [
                ...
                # Cleanup segment temp dirs
                shutil.rmtree(seg_frames_dir, ignore_errors=True)
                shutil.rmtree(seg_cropped_dir, ignore_errors=True)
                shutil.rmtree(seg_mask_dir, ignore_errors=True)
                shutil.rmtree(seg_output_dir, ignore_errors=True)
                shutil.rmtree(seg_cleaned_dir, ignore_errors=True)

                # Advanced memory management after segment cleanup
                clear_gpu_memory()
