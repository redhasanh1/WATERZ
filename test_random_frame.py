...
# Save test frame
cv2.imwrite('test_frame_random.jpg', frame)
...
if best_confidence < 0.5:
    ...
    print("Template matching may not work well on all frames")
    exit()
...
cv2.imwrite('detection_random.jpg', detection)
print(f"Saved detection_random.jpg - green box shows what will be removed")

# SD inpainting
print("\nRemoving watermark with Stable Diffusion...")
inpainter = SDInpainter(device='cuda')
result = inpainter.inpaint_region(frame, mask)

# Save
cv2.imwrite('result_random.jpg', result)
print("✅ Saved result_random.jpg")

# Comparison
comparison = np.hstack([frame, result])
cv2.imwrite('comparison_random.jpg', comparison)
print("Saved comparison_random.jpg (before | after)")
print(f"\nFrame {random_frame_num} processed successfully!")
