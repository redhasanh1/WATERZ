...
def process_segment_task(self, segment_data):
    ...
    try:
        ...
        # Use GPU encoding (h264_nvenc) for 10x speedup - critical for 1-core CPU!
        import torch
        use_gpu_encode = torch.cuda.is_available()

        if use_gpu_encode:
            # GPU encoding with NVENC (separate chip, won't affect ProPainter)
            encode_cmd = [
                'ffmpeg', '-y', '-framerate', str(fps),
                '-i', os.path.join(seg_cleaned_dir, '%04d.png'),
            ...
            print(f"   🚀 Using GPU encoding (NVENC)")
        else:
            ...
            print(f"   💻 Using CPU encoding (fallback)")

        subprocess.run(encode_cmd, capture_output=True, check=True)

        # Skip upload if on same machine (save 1-2s per segment!)
        api_base = segment_data.get('api_base') or os.getenv('API_BASE_URL') or os.getenv('TUNNEL_URL') or origin_base

        # Check if API is on same machine
        is_local = (
            api_base is None or
            'localhost' in (api_base or '').lower() or
            '127.0.0.1' in (api_base or '')
        )

        if not is_local and api_base:
            # Only upload if API is on different machine
            try:
                upload_url = f"{api_base.rstrip('/')}/api/upload-segment"
                print(f"   ⬆️  Uploading segment {seg_idx+1} to remote API...")
